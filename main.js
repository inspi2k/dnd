...
if (
  getCookie('dnd_user') == '' ||
  getCookie('dnd_team') == '' ||
  getCookie('dnd_user') == null ||
  getCookie('dnd_team') == null
) {
    ...
    if (
      ...
      const members = getSheetMember().then(values => {
        ...
        for (let i = 0; i < values.length; i++) {
          ...
          if (
            loginForm.userid.value.trim() == values[i].id &&
            loginForm.password.value.trim() == values[i].password
          ) {
            isMember = true;
        ...
        if (isMember == false)
          errorMsg('p', 'login-error-msg', 'Invalid user id and password');
      });
    }
  });
  ...
  document.querySelector('#join-ok-button').addEventListener('click', e => {
    ...
    if (
      ...
      loginForm.userteam.value.trim() == ''
    ) {
      errorMsg('p', 'login-error-msg', 'Please fill all inputs');
    } else if (
      loginForm.userid.value.trim() != loginForm.password.value.trim()
    ) {
      errorMsg('p', 'login-error-msg', 'Password must same with id');
    } else {
      // gogle sheet - member 읽어와서 대조
      const members = getSheetMember()
        .then(values => {
          for (let i = 0; i < values.length; i++) {
            if (
              loginForm.userid.value.trim() == values[i].id &&
              loginForm.username.value.trim() == values[i].name &&
              loginForm.userteam.value.trim() == values[i].team
            ) {
              errorMsg(
                'p',
                'login-error-msg',
                'duplicated id, username and team'
              );
        ...
        .then(result => {
          ...
          if (result == true) {
            ...
            const res = gsSheetWrite('member', {
              userid: loginForm.userid.value.trim(),
              username: loginForm.username.value.trim(),
              userteam: loginForm.userteam.value.trim(),
            });
            if (res == true) {
              // console.log(`환영합니다. ${loginForm.username.value.trim()}님\n\n ID로 로그인 해주세요`);
              alert(
                `환영합니다. ${loginForm.username.value.trim()}님\n\nID로 로그인 해주세요`
              );
              location.reload();
            }
          }
  ...
  // ID 찾기 처리 루틴
  document
    .querySelector('#forgot-id-ok-button')
          ...
          for (let i = 0; i < values.length; i++) {
            if (
              ...
              errorMsg(
                'p',
                'login-error-msg',
                "can't find username and userteam"
              );
              loginForm.username.value = '';
              loginForm.userteam.value = '';
            }
...
async function getSheetMember() {
  ...
  const members = parsed.table.rows
    .map(({ c }) => cleanRow(c))
    .map(([id, password, name, team, join, last]) => ({
      id,
      password,
      name,
      team,
...
async function getSheetDataUser() {
  ...
  const query = encodeURIComponent(
    `SELECT * WHERE C = "${username}" AND D = "${userteam}" AND A = DATE "${formatDate(
      new Date(),
      'YYYY-MM-dd'
    )}"`
  ).replace(/%20/gi, '+');
  ...
  const userinfo = parsed.table.rows
    .map(({ c }) => cleanRow(c))
    .map(([date, time, name, team, point, store, keyword, category]) => ({
      date,
      time,
      name,
      team,
  ...
  await Promise.all([user, list])
    .then(result => {
      ...
      for (let i = 1; i < items.length; i++) {
        const cc = user.findIndex(
          c =>
            c.store == items[i].name &&
            c.keyword == items[i].keyword &&
            c.category == items[i].category
        );
        // if( cc > -1 ) {
        ...
        if (items[i].typeTraffic == '1') {
          ...
          if (cc == -1) {
            ...
            if (items[i].ctMid != '_')
              li_a.setAttribute('data-t', items[i].ctMid);
    ...
    .finally(() => {
      ...
      document.querySelector('.user .name').innerHTML =
        '<a href="javascript:delCookie(\'dnd_user\');delCookie(\'dnd_team\');window.location.reload();" title="로그아웃"><span>' +
        username +
        '</span>님(<span>' +
        userteam +
        '</span>팀)</a>';
...
async function crUrl(event) {
  ...
  const keyword = event.currentTarget.getAttribute('data-k');
  const store = event.currentTarget.getAttribute('data-s');
  ...
  if (
    ...
    navigator.userAgent.indexOf('Table') != -1
  ) {
    base = 'https://msearch.shopping.naver.com/api/search/all?origQuery=';
  } else {
    base =
      'https://search.shopping.naver.com/api/search/all?eq=&frm=NVSHATC&iq=&window=&xq=&origQuery=';
  }
  let url = '';
  ...
  if (category == 'Shopping') {
    ...
    for (let page = 1; page < 20; page++) {
      ...
      for (let i = 0; i < items.length; i++) {
        if (nvmid == items[i].id || ctmid == items[i].id) {
          // console.log(items[i]);
          ifrm.src = items[i].crUrl;
          // console.log(event);
          toggleActive(parentNode, 'active');
          progresstime.innerHTML = '';
          progressbar.value = 0;
          progressbar.max = timemax;
          clearInterval(timer);
...
function gsSheetWrite(sheet, node_a) {
  const url_gs =
    'https://script.google.com/macros/s/AKfycbyxB5cxi5Zf4VGR3eW6ujGKzY7_mu-9H-JR7auakBL_WBYAMEPmckG1GM9Q_qrlLBEy/exec' +
    '?sheet=' +
    sheet;
...
function errorMsg(el, nodename, msg) {
  ...
  if (err_msg_el == null) {
    ...
    document
      .querySelector('#forgot-id')
      .parentNode.insertBefore(
        err_msg_el,
        document.querySelector('#forgot-id').nextSibling
      );
  }
